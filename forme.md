...
The agent's internal state is governed by nine neurochemicals: **Dopamine**, **Cortisol**, **Oxytocin**, **Serotonin**, **Norepinephrine**, **Adrenaline**, **Acetylcholine**, **Melatonin**, and **Endorphins**.
* **Chemical Objects & Registry**: The chemicals are represented as **[Chemical](file:///home/gabrialdeora/D-drive/Brain-Simulator/chemicals/models.py#L3)** and **[ChemicalRegistry](file:///home/gabrialdeora/D-drive/Brain-Simulator/chemicals/registry.py#L4)** objects, implementing a dict-like interface for backward compatibility.
* **Interactions**: Every cycle, chemicals influence each other according to a matrix defined in [chemicals.yaml](file:///home/gabrialdeora/D-drive/Brain-Simulator/config/chemicals.yaml) (e.g., Cortisol suppresses Dopamine; Serotonin and Oxytocin buffer Cortisol).
...
* **Fatigue-Driven Sleep Cycles**: If `fatigue > 0.85`, the brain enters a 5-step `sleep_cycle()` down-regulation state:
  * **SWS Phase**: Replays significant autobiography events to update striatal Q-values offline, while executing **Synaptic Pruning** (multiplying all Q-values by `0.98`).
  * **REM Phase**: Consolidates active schemas in the belief engine and posts blended dream thoughts (source `"dream"`) to the workspace.
  * **Physiological Recovery**: Decays fatigue, clears cortisol, and replenishes serotonin to baseline. Gates out incoming perceptions unless a high-intensity threat is detected, triggering an immediate wakeup.
...
- **DMN vs. TPN Network Gating (Salience Network)**:
  * **Task Positive Network (TPN)**: Triggered by high NE or active perception cycles. Focuses attention on external inputs, suppressing internal mind-wandering thoughts.
  * **Default Mode Network (DMN)**: Active during low NE and quiet cycles. Mind-wandering is enabled, and internal reflection/memory thoughts receive a `1.3x` activation boost in the workspace.
...
### H. Hippocampal Replay & Hopfield CA3 Attractor (`core/internal_thoughts.py`)
- **CA3 Hopfield Attractor Memory**: Stores experienced states (binarized chemicals and traits) into a $9 \times 9$ connection matrix $W$ via Hebbian updates during memory formation.
- **Dreaming & Mind-Wandering**: Mind-wandering is gated by DMN activation. When active, the brain maps current state parameters into a binary query vector and iterates it through Hopfield attractor equations (up to 5 cycles) to converge on a stored episodic memory trace, replaying it in the workspace if similarity overlap exceeds `7/9` dimensions. Fallback queries default to curiosity templates.
...
| :--- | :--- | :--- |
| **Dopamine** | Midbrain (VTA/SNc). Signals reward prediction error, motivation, reinforcement learning. | Modeled in [chemicals.yaml](file:///home/gabrialdeora/D-drive/Brain-Simulator/config/chemicals.yaml). Phasic updates based on Reward Prediction Errors (RPE) update striatal Q-values. Drives internal thought generation and action suggestions. |
...
| **Global Workspace (Attention)** | Frontoparietal network & thalamocortical loops. Broadcasts relevant stimuli to the whole cortex. | [GlobalWorkspace](file:///home/gabrialdeora/D-drive/Brain-Simulator/core/attention.py) class. A competitive queue where thoughts compete via emotional, novelty, goal, and recency weights. |
| **Episodic Memory** | Hippocampus & medial temporal lobe. Stores specific personal experiences. | [AutobiographicalMemory](file:///home/gabrialdeora/D-drive/Brain-Simulator/cognition/autobiographical_memory.py) class. Stores event logs, chemical snapshots, and identity states. Serializes to JSON. |
| **Cognitive Schemas** | Prefrontal & association cortices. Belief frameworks constructed from repeated events. | [BeliefEngine](file:///home/gabrialdeora/D-drive/Brain-Simulator/cognition/belief_engine.py). Computes statistical thresholds over a sliding window of events to activate cognitive rules. |
| **Maturity & Development** | Myelination & synaptic pruning. Prefrontal cortex maturation over a lifecycle. | [DynamicDevelopment](file:///home/gabrialdeora/D-drive/Brain-Simulator/core/development.py) class. Monotonic maturity growth modulated by stress exposure and reflection depth. |
| **Regret & Executive Control** | Orbitofrontal cortex (OFC) & Anterior Cingulate Cortex (ACC). Evaluates counterfactual outcomes. | [SelfReflection](file:///home/gabrialdeora/D-drive/Brain-Simulator/core/self_reflection.py) class. Calculates counterfactual differences between chosen and alternative action rewards. |
| **Speech Regulation** | Broca's area & motor cortex. Chemical modulation of vocal rate/volume (e.g. anxious stuttering). | `regulate_speech` in [brain.py](file:///home/gabrialdeora/D-drive/Brain-Simulator/core/brain.py). Modulates speech rate and intensity based on cortisol/dopamine. |
| **Social Attachment** | Hypothalamic oxytocin pathways & trust-bonding networks. | [AttachmentSystem](file:///home/gabrialdeora/D-drive/Brain-Simulator/development/attachment_system.py). Tracks source-specific trust to buffer cortisol stress responses and bias prosocial actions. |
| **Curiosity Drive** | Mesolimbic dopaminergic exploration pathway. Seeking novelty and new information. | [CuriosityEngine](file:///home/gabrialdeora/D-drive/Brain-Simulator/development/curiosity_engine.py). Tracks context frequency to scale GWT novelty scores and trigger dopaminergic discovery rewards. |
| **Goal Gating** | ACC & dlPFC executive task and objective representations. | [GoalSystem](file:///home/gabrialdeora/D-drive/Brain-Simulator/development/goal_system.py). Manages active goals (Safety, Mastery, Social Connection) to scale workspace relevance and lookahead expected utilities. |
| **Adrenaline** | Adrenal medulla. Rapid survival responses, acute stress mobilization. | Gated in [chemicals.yaml](file:///home/gabrialdeora/D-drive/Brain-Simulator/config/chemicals.yaml) and [DecisionEngine](file:///home/gabrialdeora/D-drive/Brain-Simulator/decision/decision_engine.py) to trigger flight-or-fight overrides. |
| **Acetylcholine** | Basal forebrain. Focused attention and cognitive plasticity (LTP). | Modulates Striatal Q-learning rate and Hopfield Hebbian updates in [VirtualBrain](file:///home/gabrialdeora/D-drive/Brain-Simulator/core/brain.py). |
| **Melatonin** | Pineal gland. Governs sleep-wake cycles and circadian rhythms. | Modulates fatigue-based sleep gating in [VirtualBrain](file:///home/gabrialdeora/D-drive/Brain-Simulator/core/brain.py). |
| **Endorphins** | Pituitary gland/hypothalamus. Opioid peptides masking physical/emotional distress. | Buffers cortisol and spikes serotonin upon regret resolution in `tick()`. |
...
### Q6: "What are the decoupled/staged systems in this codebase, and how have they been integrated?"
> **Answer:** 
> Initially designed as offline stages, all core evolutionary drives and social modeling systems are now fully integrated into the active cognitive loop of the `VirtualBrain`:
...
`AppBuilder.build_fullstack(name, kind, backend, frontend, theme)` emits a complete vertical slice for one of **eight kinds**: `food_delivery`, `ecommerce`, `booking`, `task_tracker`, `chat`, `blog`, `notes`, `fitness`.
...
3. **Table-Driven Templates**: Each kind is declared in the `FULLSTACK_KINDS` spec dict (title, tagline, cart-vs-collection `mode`, item fields, seed data, and full SQL schema). Dispatchers then render the backend and frontend:
   - `_fullstack_backend(spec, cart_mode, backend)` — the **four-framework backend dispatcher** (`flask` → `app.py`, `django` → `manage.py` + `config/` project, `express`/`fastify` → `server.js`) builds one shared REST API contract on every stack: pbkdf2-hashed session auth, `?q=` search + `?limit/?offset=` pagination, cart/orders (cart kinds), private/public collection CRUD (collection kinds), Server-Sent Events streaming for chat, JSON 404s for unknown `/api/*`, and the SPA fallback route. SQLite is the zero-config dev DB (WAL mode, busy timeout, `DATABASE_PATH` override); a `DATABASE_URL` env var switches any backend to PostgreSQL (`SERIAL PRIMARY KEY` schema, `?`→`$n`/`%s` param translation in the data layer).
   - `_fullstack_frontend(spec, name, cart_mode, theme, frontend)` — `single` renders a single-page HTML/JS UI wired to the API; `react` renders a full React + Vite SPA (`src/`, `vite.config.js`, Vite proxy) that the backend serves from `dist/`.
4. **Docker Delivery**: Every project includes `backend/requirements.txt` (Python) or `package.json` (Node), a multi-worker `gunicorn`/Node Dockerfile, `docker-compose.yml` (web on `:8000` Python / `:3000` Node, plus an optional `postgres:16` service), and a per-app `README.md` with dev and production-ish run steps.

### B. How the Deterministic Debugger Works (`debug_app`)

`debug_app(name, fix=False)` performs static analysis over the generated project (framework-aware: it locates `backend/app.py`, `backend/manage.py`, or `backend/server.js`), plus `backend/schema.sql` and the frontend, and returns a structured report (`{ok, bugs[{severity, location, message}], fixed[]}`).
...
| Syntax errors | Python: `compile(...)`. Node: `node --check` when available |
...
| Broken frontend routes | `_frontend_calls` parses each `api(...)` call with balanced-paren extraction (so a later call's options can't leak its `method:` into an earlier call), normalizes dynamic segments to `{}` wildcards, then matches against backend routes with method/verb awareness |
| Missing DB bootstrap | sqlite3/pg present ⇒ an `_init_db()` definition and a non-empty `schema.sql` are required |
...
> **Answer:** A complete, runnable vertical app — REST backend in any of four frameworks (Flask, Django, Express, Fastify), SQL schema (SQLite by default, Postgres via `DATABASE_URL`), single-file or React+Vite frontend, Docker deployment files, and README — with auth, payments, search + pagination, and (for chat) SSE. Repairs are proven the same way the generator is verified: rebuild deterministically, run the app's live test client (register → authenticate → create/read → verify authz boundaries such as delete-others-fails), and assert the debugger now reports zero bugs. The generated README even documents migrating the SQLite schema to PostgreSQL for larger scale.
