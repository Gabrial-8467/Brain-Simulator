...
class SleepManager:
        ...
        if os.path.exists(sleep_config_path):
            try:
                with open(sleep_config_path, "r") as f:
                    ...
                    if sleep_cfg:
                        self.cycle_period = int(sleep_cfg.get("cycle_period", self.cycle_period))
                        self.sleep_duration = int(sleep_cfg.get("sleep_duration", self.sleep_duration))
                        self.fatigue_accumulation_rate = float(sleep_cfg.get("fatigue_accumulation_rate", self.fatigue_accumulation_rate))
                        self.fatigue_recovery_rate = float(sleep_cfg.get("fatigue_recovery_rate", self.fatigue_recovery_rate))
            except Exception:
                pass

    # -----------------------------------------
    # SLEEP DRIVE DYNAMICS
    # -----------------------------------------
    ...
    def update_sleep_drives(self, brain: Any) -> None:
        ...
        if brain.sleeping:
            self.process_s = max(0.0, self.process_s - self.fatigue_recovery_rate)
        else:
            self.process_s = min(1.0, self.process_s + self.fatigue_accumulation_rate)
    ...
    def run_sleep_cycle(self, brain: Any) -> dict[str, Any]:
        ...
        # Determine phase division (20% REM at the end of the sleep cycle)
        rem_threshold = max(2, self.sleep_duration // 5)

        # Offline Consolidation Phase (SWS)
        if brain.sleep_ticks_left >= rem_threshold:
